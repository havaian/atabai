                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <Select
                            ...
                            :placeholder="$t('history.filters.allTemplates')"
                    ...
                    <div>
                        ...
                        <Select
                            ...
                            :placeholder="$t('history.filters.allDates')"
                                ...
                                <div class="col-span-2">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <button v-if="file.status === 'completed'" 
                                            ...
                                            <div v-else class="h-4 w-4 animate-spin border-2 border-green-600 border-t-transparent rounded-full"></div>
                                        </button>

                                        <!-- Retry Button -->
                                        <button v-if="file.status === 'failed'" 
                                            ...
                                            <div v-else class="h-4 w-4 animate-spin border-2 border-yellow-600 border-t-transparent rounded-full"></div>
                                        </button>
                                        ...
                                        <button @click="deleteFile(file)"
                                            ...
                                            <div v-else class="h-4 w-4 animate-spin border-2 border-red-600 border-t-transparent rounded-full"></div>
                                        </button>
                                    </div>
                                </div>
                            </div>
...
async function downloadFile(file) {
    try {
        ...
        await filesStore.downloadFile(file.id, true) // true for processed file
    } catch (err) {
...
async function retryFile(file) {
    try {
        file.retrying = true
        // TODO: Implement retry functionality when retryProcessing method is available
        // await filesStore.retryProcessing(file.id)
        console.log('Retry functionality not yet implemented')
        // For now, just refresh the file data
        await fetchFiles()
    } catch (err) {
...
async function deleteFile(file) {
    ...
    try {
        file.deleting = true
        // TODO: Implement delete functionality when deleteFile method is available
        // await filesStore.deleteFile(file.id)
        console.log('Delete functionality not yet implemented')
        // For now, just remove from local list (temporary)
...
async function fetchFiles() {
    try {
        ...
        // Use the existing fetchRecentFiles method
        await filesStore.fetchRecentFiles()
        
        // Get files from the store
        files.value = filesStore.recentFiles.map(file => ({
            ...file,
            downloading: false,
            retrying: false,
            deleting: false
        ...
        error.value = t('history.error.fetchFailed')
    } finally {
